...
class ConvBatchLeakyReLU(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, separable=False):
        super(ConvBatchLeakyReLU, self).__init__()
        if separable:
            self.conv = SeparableConv2D(in_channels, out_channels, kernel_size, stride=stride, padding=padding)
        else:
